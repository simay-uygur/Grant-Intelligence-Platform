...
import { FileText, KanbanSquare, Landmark, MessagesSquare } from "lucide-react";
...
export function LoginPage({ onSignIn }: { onSignIn: () => void }) {
  ...
  return (
    <div className="flex min-h-screen bg-background">
      {/* Branded panel — the app's dark sidebar treatment, desktop only. */}
      <div className="hidden w-full max-w-md flex-col justify-between bg-sidebar px-10 py-12 text-sidebar-foreground lg:flex xl:max-w-lg">
        <div>
          ...
          <p className="mt-12 max-w-sm text-balance text-2xl font-bold leading-snug">
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-4 py-12 sm:px-6">
        ...
        <div className="w-full max-w-sm rounded-2xl border border-border bg-card p-6 text-card-foreground shadow-sm sm:p-8">
          ...
          <form onSubmit={handleSubmit} noValidate className="mt-6 space-y-4">
            ...
            <Button
              type="submit"
              className="w-full rounded-lg bg-brand text-brand-foreground shadow-sm hover:bg-brand/90"
            >
              Sign in
            </Button>
          </form>
