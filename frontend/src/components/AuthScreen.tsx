...
export function AuthScreen() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (!response.ok || !payload.token) throw new Error(payload.detail ?? "Unable to authenticate.");
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-background px-4">
      <form onSubmit={submit} className="w-full max-w-sm space-y-5 rounded-2xl border border-border bg-card p-7 shadow-sm">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand">Grant Navigator</p>
          <h1 className="mt-2 text-2xl font-semibold text-foreground">
            {mode === "login" ? "Welcome back" : "Create your account"}
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {mode === "login" ? "Log in to access your grant workspace." : "Your saved work belongs to your account."}
          </p>
        </div>
